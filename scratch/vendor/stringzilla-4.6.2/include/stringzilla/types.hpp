...
struct error_costs_unary_t {
    SZ_DYNAMIC constexpr error_cost_t operator()(char a, char b) const noexcept { return a == b ? 0 : 1; }
    SZ_DYNAMIC constexpr error_cost_t operator()(sz_rune_t a, sz_rune_t b) const noexcept { return a == b ? 0 : 1; }
    SZ_DYNAMIC constexpr sz_size_t magnitude() const noexcept { return 1; }
};
...
#endif
        scalar_type_
        round_up_to_multiple(scalar_type_ x, scalar_type_ divisor) {
    sz_assert_(divisor > 0 && "Divisor must be positive");
...
#endif
        void
        trivial_swap(value_type_ &x, value_type_ &y) noexcept {
