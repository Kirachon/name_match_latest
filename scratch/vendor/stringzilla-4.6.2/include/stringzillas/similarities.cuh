...
__global__ void linear_score_across_cuda_device_(              //
    ...
    // Correctness-first fallback for short Levenshtein tasks on native Windows CUDA.
    //
    // The cooperative device-level scorer is the path selected for our short
    // name strings. On the Windows/MSVC/CUDA build it currently reports illegal
    // memory access before the warp-level fallback can run, so handle bounded
    // short strings with a single-lane Wagner-Fischer DP.
    if (shorter_length < 128) {
        if (is_main_thread) {
            score_t previous_local[128];
            score_t current_local[128];
            for (index_t j = 0; j <= shorter_length; ++j)
                previous_local[j] = static_cast<score_t>(j * gap_costs.open_or_extend);

            for (index_t i = 1; i <= longer_length; ++i) {
                current_local[0] = static_cast<score_t>(i * gap_costs.open_or_extend);
                for (index_t j = 1; j <= shorter_length; ++j) {
                    score_t const substitution =
                        previous_local[j - 1] +
                        static_cast<score_t>(substituter(shorter_ptr[j - 1], longer_ptr[i - 1]));
                    score_t const deletion = previous_local[j] + static_cast<score_t>(gap_costs.open_or_extend);
                    score_t const insertion = current_local[j - 1] + static_cast<score_t>(gap_costs.open_or_extend);
                    current_local[j] =
                        min_or_max<objective_k>(substitution, min_or_max<objective_k>(deletion, insertion));
                }
                for (index_t j = 0; j <= shorter_length; ++j) previous_local[j] = current_local[j];
            }
            *result_ptr = static_cast<final_score_t>(previous_local[shorter_length]);
        }
        return;
    }
...
__global__ void linear_score_on_each_cuda_warp_(                             //
    ...
    for (size_t task_idx = global_warp_index; task_idx < tasks_count; task_idx += warps_per_device) {
        ...
        // Correctness-first fallback for the native Windows CUDA probe.
        //
        // The optimized diagonal warp scorer currently returns length-like values
        // on this Windows/MSVC/CUDA path, even for exact matches. Use a simple
        // Wagner-Fischer DP in one lane so the CUDA backend can be validated
        // independently from the broken warp-vectorized scorer.
        if (shorter_length < 128) {
            if (is_main_thread) {
                score_t previous_local[128];
                score_t current_local[128];
                for (unsigned j = 0; j <= shorter_length; ++j)
                    previous_local[j] = static_cast<score_t>(j * gap_costs.open_or_extend);

                for (unsigned i = 1; i <= longer_length; ++i) {
                    current_local[0] = static_cast<score_t>(i * gap_costs.open_or_extend);
                    for (unsigned j = 1; j <= shorter_length; ++j) {
                        score_t const substitution =
                            previous_local[j - 1] +
                            static_cast<score_t>(substituter(shorter_global[j - 1], longer_global[i - 1]));
                        score_t const deletion = previous_local[j] + static_cast<score_t>(gap_costs.open_or_extend);
                        score_t const insertion = current_local[j - 1] + static_cast<score_t>(gap_costs.open_or_extend);
                        current_local[j] =
                            min_or_max<objective_k>(substitution, min_or_max<objective_k>(deletion, insertion));
                    }
                    for (unsigned j = 0; j <= shorter_length; ++j) previous_local[j] = current_local[j];
                }
                result_ref = previous_local[shorter_length];
            }
            __syncwarp();
            continue;
        }
...
struct levenshtein_distances<char_type_, gap_costs_type_, allocator_type_, capability_,
    ...
    cuda_status_t operator()(                                                                 //
        ...
        auto score_short_task_on_host = [&](task_t const &task) noexcept -> size_t {
            if constexpr (!is_affine_k) {
                size_t previous_local[128];
                size_t current_local[128];
                for (size_t j = 0; j <= task.shorter_length; ++j)
                    previous_local[j] = j * gap_costs_.open_or_extend;

                for (size_t i = 1; i <= task.longer_length; ++i) {
                    current_local[0] = i * gap_costs_.open_or_extend;
                    for (size_t j = 1; j <= task.shorter_length; ++j) {
                        size_t const substitution =
                            previous_local[j - 1] + substituter_(task.shorter_ptr[j - 1], task.longer_ptr[i - 1]);
                        size_t const deletion = previous_local[j] + gap_costs_.open_or_extend;
                        size_t const insertion = current_local[j - 1] + gap_costs_.open_or_extend;
                        current_local[j] = sz_min_of_two(substitution, sz_min_of_two(deletion, insertion));
                    }
                    for (size_t j = 0; j <= task.shorter_length; ++j) previous_local[j] = current_local[j];
                }
                return previous_local[task.shorter_length];
            }
            else {
                sz_unused_(task);
                return 0;
        ...
        for (size_t i = 0; i < first_strings.size(); ++i) {
            ...
            if constexpr (!is_affine_k) {
                if (task.shorter_length < 128) {
                task.result = score_short_task_on_host(task);
                task.density = infinite_warps_per_multiprocessor_k;
                count_empty_tasks++;
                }
            }
            else if (task.density == infinite_warps_per_multiprocessor_k) {
        ...
        if (device_level_tasks.size()) {
            ...
            safe_vector<final_score_t, scores_allocator_t> device_level_results_buffer(alloc_);
            if (device_level_results_buffer.try_resize(device_level_tasks.size()) == status_t::bad_alloc_k)
                return {status_t::bad_alloc_k};
            ...
            for (size_t i = 0; i < device_level_tasks.size(); ++i) {
                task_t &task = device_level_tasks[i];
                auto *device_result_ptr = device_level_results_buffer.data() + i;
                ...
                device_level_kernel_args[4] = (void *)(&device_result_ptr);
            ...
            for (size_t i = 0; i < device_level_tasks.size(); ++i)
                device_level_tasks[i].result = device_level_results_buffer[i];
        }
...
struct cuda_nw_or_sw_byte_level_scores_ {
    ...
    cuda_status_t operator()(                                                                 //
        ...
        if (device_level_tasks.size()) {
            ...
            for (size_t i = 0; i < device_level_tasks.size(); ++i) {
                task_t &task = device_level_tasks[i];
                auto *device_result_ptr = &task.result;
                device_level_kernel_args[0] = (void *)(&task.shorter_ptr);
                device_level_kernel_args[1] = (void *)(&task.shorter_length);
                device_level_kernel_args[2] = (void *)(&task.longer_ptr);
                device_level_kernel_args[3] = (void *)(&task.longer_length);
