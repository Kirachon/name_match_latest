...
fn build_stringzillas(serial_flags: &HashMap<String, bool>) {
    ...
    if is_cuda {
        ...
        build.define("SZ_USE_KEPLER", "0");
        build.define("SZ_USE_HOPPER", "0");
        build.flag("-std=c++20");
        build.flag_if_supported("/std:c++20");
        build.flag("--expt-relaxed-constexpr");
        build.flag("-arch=sm_89");
    }
    // AMD GPU backend
    else if is_rocm {
        build.cpp(true);
    ...
    if let Err(selected_backend_error) = build.try_compile("stringzillas") {
        if is_cuda {
            panic!("Failed to compile stringzillas CUDA backend: {selected_backend_error:?}");
        }
